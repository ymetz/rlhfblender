...
from rlhfblender.data_models.feedback_models import (
    StandardizedFeedback, UnprocessedFeedback, FeedbackType, SimplifiedFeedbackType,
    Rating, Ranking, Correction, Demonstration, FeatureSelection,
    TextFeedback, MetaFeedback, Target, Episode, State, Segment, Entire,
    Origin
)
...
class FeedbackTranslator:
    ...
    def _create_target(self, target_dict: dict, granularity: str) -> Optional[Target]:
        ...
        if granularity == "episode":
            ...
            return State(
                **base_kwargs,
                step=target_dict.get("step", -1)
            )
        elif granularity == "segment":
            return Segment(
                **base_kwargs,
                start=target_dict.get("start", -1),
                end=target_dict.get("end", -1)
            )
    ...
    def give_feedback(self, session_id: str, feedback: UnprocessedFeedback) -> StandardizedFeedback:
        ...
        if feedback.feedback_type == FeedbackType.rating:
            ...
            targets = [self._create_target(feedback.targets[0], feedback.granularity)] if feedback.targets else []
            content = FeatureSelection(
                features=feedback.feature_selection or [],
                importance=feedback.feature_importance
            )
    ...
    def get_unified_dataset(self, n_feedback: int = -1, **dataset_kwargs) -> 'UnifiedFeedbackDataset':
        ...
        return UnifiedFeedbackDataset(
            feedbacks=processed_feedback,
            n_feedback=n_feedback,
            env_name=str(self.env) if self.env else "",
            **dataset_kwargs
        )
    
    @staticmethod
    def create_unified_dataset_from_processed(processed_feedback: list['StandardizedFeedback'], 
                                            n_feedback: int = -1, 
                                            env_name: str = "",
                                            **dataset_kwargs) -> 'UnifiedFeedbackDataset':
        """
