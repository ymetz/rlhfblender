...
def update_result(result_file, **kwargs):
    ...
    if os.path.exists(result_file):
        ...
        result = {"uncertainty": 0.0, "avg_predicted_reward": 0.0, "training_loss": 0.0,
                  "validation_loss": 0.0, "training_complete": False, "trained_model_path": "",
                  "new_episodes": [], "metrics": {}}
...
def main():
    ...
    try:
        ...
        for feedback_type in feedback_types:
            ...
            type_feedback = [item for item in feedback_data if 
                            item.get("feedback_type", "evaluative") == feedback_type]
            ...
            train_set, val_set = random_split(
                dataset, 
                lengths=[training_set_size, len(dataset) - training_set_size]
            )
        ...
        episode_cmd = [
            "python", os.path.join(os.path.dirname(__file__), "collect_episodes.py"),
            "--session-dir", str(session_dir),
            "--env", args.env,
            "--num-episodes", "10",
            "--random-policy", "false" if model_path else "true",
            "--reward-model-path", model_path if model_path else ""
        ]
        
        # Run process and wait for completion
        episode_log = open(session_dir / "episode_collection.log", "w")
        episode_process = subprocess.Popen(
            episode_cmd,
            stdout=episode_log,
