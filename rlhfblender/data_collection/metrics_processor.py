...
def process_metrics(benchmark_results: RecordedEpisodesContainer) -> dict:
    ...
    avg_entropy = np.mean(
        [info.get("entropy", 0.0) for info in benchmark_results.infos]
    )
    avg_value = np.mean([info.get("value", 0.0) for info in benchmark_results.infos])
    avg_action_prob = (
        1.0
        if len(benchmark_results.probs.shape) == 1
        else np.mean(np.max(benchmark_results.probs, axis=1)).astype(float)
    )
    if len(benchmark_results.rewards) > 0:
        avg_reward_freq = (
            np.count_nonzero(benchmark_results.rewards)
            / benchmark_results.rewards.shape[0]
        )
    else:
        avg_reward_freq = 0.0
