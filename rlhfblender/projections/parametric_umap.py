...
class EdgeDataset(Dataset):
    ...
    def __init__(self, X, head, tail, weights=None, parametric_embedding=True, parametric_reconstruction=False):
        """
...
class ParametricUMAP(UMAP):
    def __init__(
        ...
        **kwargs,
        ...
        if parametric_reconstruction and not parametric_embedding:
            warn("Parametric decoding is not implemented with nonparametric " "embedding. Turning off parametric decoding")
    ...
    def fit(self, X, y=None, precomputed_distances=None, landmark_positions=None):
        ...
        if (self.prev_epoch_X is not None) & (landmark_positions is None):
            ...
            landmark_positions = np.stack([np.array([np.nan, np.nan])] * X.shape[0] + list(self.transform(self.prev_epoch_X)))
        ...
        if landmark_positions is not None:
            ...
            if len_X != len_land:
                raise ValueError(
                    f"Length of X = {len_X}, length of landmark_positions " f"= {len_land}, while it must be equal."
                )

        if self.metric == "precomputed":
            if precomputed_distances is None:
                raise ValueError("Precomputed distances must be supplied if metric " "is precomputed.")
            # prepare X for training the network
            self._X = X
            # generate the graph on precomputed distances
            return super().fit(precomputed_distances, y, landmark_positions=landmark_positions)
        else:
    ...
    def fit_transform(self, X, y=None, precomputed_distances=None, landmark_positions=None):
        ...
        if self.metric == "precomputed":
            ...
            self._X = X
            # generate the graph on precomputed distances
            return super().fit_transform(precomputed_distances, y, landmark_positions=landmark_positions)
        else:
    ...
    def _define_model(self):
        ...
        if self.optimizer_class:
            self.optimizer = self.optimizer_class(self.parametric_model.parameters(), lr=self.lr)
        elif not hasattr(self, "optimizer"):
            self.optimizer = torch.optim.Adam(self.parametric_model.parameters(), lr=1e-3)
    ...
    def _fit_embed_data(self, X, n_epochs, init, random_state, landmark_positions=None):
        ...
        if self.parametric_reconstruction and (np.max(X) > 1.0 or np.min(X) < 0.0):
            warn("Data should be scaled to the range 0-1 for cross-entropy reconstruction loss.")
        ...
        if self.parametric_reconstruction and self.reconstruction_validation is not None:
            # reshape data for network
            if len(self.dims) > 1:
                validation_data = np.reshape(
                    self.reconstruction_validation, [len(self.reconstruction_validation)] + list(self.dims)
                )
            else:
        ...
        for step, (data, weight) in enumerate(dataloader):
            ...
            if landmark_positions is not None:
                ...
                batch_indices = torch.arange(step * self.batch_size, min((step + 1) * self.batch_size, len(head)))
                landmark_data = torch.tensor(landmark_positions[batch_indices], dtype=torch.float32).to(self.device)

            # Compute loss
            loss = self.parametric_model(head_sample, tail_sample, weight=weight, landmark_data=landmark_data)
            ...
            if step % steps_per_epoch == 0:
                ...
                if validation_tensor is not None:
                    with torch.no_grad():
                        ...
                        val_loss = F.binary_cross_entropy_with_logits(reconstructions, validation_tensor)
    ...
    def _construct_edge_dataset(self, X, graph_, n_epochs, batch_size, landmark_positions=None):
        ...
        # Create DataLoader
        dataloader = DataLoader(edge_dataset, batch_size=batch_size, shuffle=True, drop_last=True, num_workers=0)
...
def load_ParametricUMAP(save_location, verbose=True):
    ...
    if os.path.exists(parametric_model_output):
        ...
        model.parametric_model.load_state_dict(torch.load(parametric_model_output, map_location=device))
        if verbose:
...
class UMAPModel(nn.Module):
    ...
    def forward(self, head_input, tail_input, weight=None, landmark_data=None):
        ...
        if self.parametric_reconstruction:
            ...
            recon_loss = self.parametric_reconstruction_loss_fn(embedding_to_recon, head_input)
    ...
    def _umap_loss(self, embedding_to, embedding_from, repulsion_strength=1.0):
        ...
        probabilities_distance = self._convert_distance_to_probability(distance_embedding, self.a, self.b)
        ...
        attraction_loss, repellant_loss, ce_loss = self._compute_cross_entropy(
            probabilities_graph, probabilities_distance, repulsion_strength=repulsion_strength
        )
    ...
    def _compute_cross_entropy(self, probabilities_graph, probabilities_distance, eps=1e-4, repulsion_strength=1.0):
        """
    ...
    if encoder is None:
        if parametric_embedding:
            ...
            if init_embedding is not None:
                ...
                embedding_layer.weight.data = torch.tensor(init_embedding, dtype=torch.float32)
