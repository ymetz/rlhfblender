...
class InverseProjectionHandler:
    ...
    def precompute_bivariate_surface(
        ...
        pred_values : prediction values at the grid points
        unc_values : uncertainty values at the grid points
        ...
        zi_pred = griddata(coords, pred, (xi_grid, yi_grid), method=method)
        zi_unc = griddata(coords, unc, (xi_grid, yi_grid), method=method)

        if np.any(np.isnan(zi_pred)):
            zi_pred = np.where(np.isnan(zi_pred), griddata(coords, pred, (xi_grid, yi_grid), method="nearest"), zi_pred)
        if np.any(np.isnan(zi_unc)):
            zi_unc = np.where(np.isnan(zi_unc), griddata(coords, unc, (xi_grid, yi_grid), method="nearest"), zi_unc)

        norm_pred = Normalize(vmin=pred.min(), vmax=pred.max())
        norm_unc = Normalize(vmin=unc.min(), vmax=unc.max())
...
if __name__ == "__main__":
    ...
    # Get predictions from a dense grid
