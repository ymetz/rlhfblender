...
class ParametricAngleUMAP(umap.UMAP):
    def __init__(
        self,
        optimizer=None,
        batch_size=None,
    ...
    def fit(self, X, y=None, precomputed_distances=None, action_angles=None):

        if action_angles is not None:
            self.action_angles = tf.convert_to_tensor(
                action_angles, dtype_hint=tf.float32
            )
    ...
    def fit_transform(self, X, y=None, precomputed_distances=None, action_angles=None):
    ...
    def _define_model(self):
        ...
        #    embedding_to_from
        # )
        outputs["umap"] = tf.keras.layers.Lambda(lambda x: x, name="umap")(
            embedding_to_from
        )

        outputs["angle"] = tf.keras.layers.Lambda(lambda x: x, name="angle")(
            embedding_to
        )
    ...
    def _compile_model(self):
        ...
        if self.action_angles is not None:
            print("Use Action Angles")
            losses["angle"] = angle_loss(self.action_angles)
            loss_weights["angle"] = self.angle_loss_weight
            # loss_weights["umap"] = 1.0 - self.angle_loss_weight
...
def umap_loss(
    ...
    def loss(placeholder_y, embed_to_from):
        ...
        # Only use the first halves as samples (we only need the second half for the angle loss)
        embedding_to, _ = tf.split(embedding_to, num_or_size_splits=2, axis=0)
        embedding_from, _ = tf.split(embedding_from, num_or_size_splits=2, axis=0)
...
def angle_loss(ref_action_angles):
    """Loss based on the distance between elements in a batch"""
    ...
    def loss(y, z_x):
        ...
        # flatten data
        indices = tf.reshape(y, [-1])
        # Clip indices to prevent out of bounds errors
        indices = tf.clip_by_value(indices, 0, ref_action_angles.shape[0] - 1)

        z_x1, z_x2 = tf.split(z_x, num_or_size_splits=2, axis=0)
        # Find the index position for each increasing number in index, makes the indices sequential again
        # in terms of states in the original sequence
        sort_order = tf.argsort(indices)
        sorted_indices = tf.gather(indices, sort_order)
        sorted_z_x2 = tf.gather(z_x2, sort_order)

        # Find consecutive indices
        consecutive_indices = tf.squeeze(
            tf.where(sorted_indices == tf.roll(sorted_indices + 1, shift=1, axis=0))
        )
        # Retreive indices at the consective index positions
        reference_sort_indices = tf.gather(sorted_indices, consecutive_indices)

        # get angles
        angles = tf.math.atan2(
            tf.gather(sorted_z_x2, indices=consecutive_indices)[:, 1]
            - tf.gather(sorted_z_x2, indices=consecutive_indices - 1)[:, 1],
            tf.gather(sorted_z_x2, indices=consecutive_indices)[:, 0]
            - tf.gather(sorted_z_x2, indices=consecutive_indices - 1)[:, 0],
        )
        angles = (angles + 2 * tf.constant(np.pi)) % (2 * tf.constant(np.pi))

        # get indices for reference angles matched to tehe cumputed angles
        sorted_ref_angles = tf.gather(ref_action_angles, indices=reference_sort_indices)

        return tf.reduce_mean(
            tf.abs(
                tf.atan2(
                    tf.sin(angles - sorted_ref_angles),
                    tf.cos(angles - sorted_ref_angles),
                )
            )
        )
...
def construct_edge_dataset(
    ...
    def gather_X(edge_to, edge_from):
        ...
        if gather_indices_in_python:
            ...
            shifted_edge_to_batch = tf.py_function(
                gather_index,
                [tf.clip_by_value(edge_to - 1, 0, tf.int32.max)],
                [tf.float32],
            )[0]
            edge_from_batch = tf.py_function(gather_index, [edge_from], [tf.float32])[0]
            shifted_edge_from_batch = tf.py_function(
                gather_index,
                [tf.clip_by_value(edge_from - 1, 0, tf.int32.max)],
                [tf.int32],
            )[0]
        else:
            edge_to_batch = tf.gather(X, edge_to)
            shifted_edge_to_batch = tf.gather(
                X, tf.clip_by_value(edge_to - 1, 0, tf.int32.max)
            )
            edge_from_batch = tf.gather(X, edge_from)
            shifted_edge_from_batch = tf.gather(
                X, tf.clip_by_value(edge_from - 1, 0, tf.int32.max)
            )
        return (
            tf.concat([edge_to_batch, shifted_edge_to_batch], axis=0),
            tf.concat([edge_from_batch, shifted_edge_from_batch], axis=0),
            tf.clip_by_value(edge_to - 1, 0, tf.int32.max),
        )

    def get_outputs(edge_to_batch, edge_from_batch, indices):
        outputs = {"umap": tf.repeat(0, batch_size)}
        outputs["angle"] = indices
