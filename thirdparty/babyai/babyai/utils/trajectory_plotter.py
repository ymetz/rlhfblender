...
def set_background_image(image, plot, zoom, xy):
    ...
    ab = AnnotationBbox(
        imagebox, xy=xy, annotation_clip=False, zorder=1, pad=0, frameon=False
    )
    plot.add_artist(ab)
...
def set_legend(type, plot, uncertainty_color):
    ...
    red_triangle = mlines.Line2D(
        [], [], color="red", marker=">", markersize=5, label="initial agent position"
    )
    blue_triangle = mlines.Line2D(
        [], [], color="blue", marker=">", markersize=5, label="agent direction"
    )
    green_triangle = mlines.Line2D(
        [], [], color="green", marker=">", markersize=5, label="optimal direction"
    )
...
def set_mission(plot, mission, maximum):
    ...
    """
    factor = (
        0.25 if maximum[1] == 20 else 0.45
    )  # factor to place the mission box properly
    offsetbox = TextArea("Mission: \n" + mission)
    ab = AnnotationBbox(
        offsetbox,
        xy=(maximum[0] / 1.7, maximum[1] + (maximum[1] * factor)),
...
def place_direction_marker(
    plot, x_coord, y_coord, step_idx, directions, label, color, size
):
    """
...
def visualize_trajectory(
    ...
    (mission, x_coord, y_coord, directions, image) = collect_coordinates(
        offset, env_name, seed, actions
    )
    (_, x_coord_true, y_coord_true, directions_true, _) = collect_coordinates(
        offset_true, env_name, seed, actions_true
    )
    ...
    if step_wise and (step_idx is not None) and (color is not None):
        ...
        zoom = 6 / (max[0] + 1)
        visualize_as_glyph(
            plot, glyph_no, x_coord, y_coord, x_coord_true, y_coord_true, max
        )
...
def visualize_as_plot(
    ...
    if type.__eq__("both paths") or type.__eq__("agent path"):
        ...
        place_direction_marker(
            plot, x_coord, y_coord, step_idx, directions, "direction", "blue", max
        )
    ...
    if type.__eq__("both paths") or type.__eq__("optimal path"):
        draw_line_collection(
            plot, x_coord_true, y_coord_true, step_idx, "yellowgreen", color
        )
        place_direction_marker(
            plot,
            x_coord_true,
        ...
        if actions_true[step_idx] in [0, 1]:
            place_arrow(
                plot,
                x_coord_true[step_idx],
                y_coord_true[step_idx],
                actions_true[step_idx],
                directions_true[step_idx],
                color,
                0.6,
            )
...
def visualize_replay_trajectory(trajectory_plot, env_name, seed, actions, actions_true):
    ...
    (_, x_coord, y_coord, _, image) = collect_coordinates(
        offset, env_name, seed, actions
    )
    (_, x_coord_true, y_coord_true, directions_true, _) = collect_coordinates(
        offset_true, env_name, seed, actions_true
    ...
    draw_line_collection(
        trajectory_plot, x_coord_true, y_coord_true, 0, "yellowgreen", "yellowgreen"
    )

    #  mark agent's initial position
    size = 400 if max[0] == 6 else 50
    trajectory_plot.scatter(
        x_coord_true[0],
        y_coord_true[0],
        label="initial position",
        c="blue",
        marker=markers_true[0],
        zorder=5.0,
        s=size,
    )
    ...
    blue_triangle = mlines.Line2D(
        ...
        marker=">",
        markersize=5,
        label="agent's initial\nposition",
    )
    red_triangle = mlines.Line2D(
        [], [], color="red", marker=">", markersize=5, label="agent's final\nposition"
    )
    patch_agent = patches.Patch(color="tab:blue", label="agent path")
    patch_optimal = patches.Patch(color="yellowgreen", label="optimal path")
    trajectory_plot.legend(
        handles=[blue_triangle, red_triangle, patch_agent, patch_optimal],
        bbox_to_anchor=(1, 0),
        loc="lower left",
        fancybox=True,
        frameon=True,
