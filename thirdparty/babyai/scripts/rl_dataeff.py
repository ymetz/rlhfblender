...
parser.add_argument("--path", default=".", help="path to model logs")
parser.add_argument("--regex", default=".*", help="filter out some logs")
parser.add_argument(
    "--other", default=None, help="path to model logs for ttest comparison"
)
parser.add_argument(
    "--other_regex", default=".*", help="filter out some logs from comparison"
)
parser.add_argument(
    "--window",
    type=int,
...
def dataeff(df_model, window):
    ...
    if smoothed_sr.max() < 0.99:
        print(
            "not done, success rate is only {}% so far".format(100 * smoothed_sr.max())
        )
        return int(1e9)
...
result = {
    ...
    "mean": data.mean(),
    "std": data.std(),
    "min": data.mean() - Z * data.std(),
    "max": data.mean() + Z * data.std(),
    "fps_mean": fps.mean(),
    "fps_std": fps.std(),
}
print(result)
