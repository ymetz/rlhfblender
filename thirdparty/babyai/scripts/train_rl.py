...
parser.add_argument(
    "--reward-scale", type=float, default=20.0, help="Reward scale multiplier"
)
parser.add_argument(
    "--gae-lambda",
    type=float,
...
default_model_name = (
    "{env}_{algo}_{arch}_{instr}_{mem}_seed{seed}{info}{coef}_{suffix}".format(
        **model_name_parts
    )
)
...
try:
    last_commit = subprocess.check_output(
        "cd {}; git log -n1".format(babyai_code), shell=True
    ).decode("utf-8")
...
try:
    diff = subprocess.check_output(
        "cd {}; git diff".format(babyai_code), shell=True
    ).decode("utf-8")
    if diff:
...
while status["num_frames"] < args.frames:
    ...
    if args.save_interval > 0 and status["i"] % args.save_interval == 0:
        ...
        if save_model:
            ...
            logger.info(
                "Return {: .2f}; not the best model; not saved".format(mean_return)
            )
