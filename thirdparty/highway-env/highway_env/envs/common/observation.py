...
from highway_env.utils import Vector, distance_to_circle
...
class OccupancyGridObservation(ObservationType):
    ...
    def __init__(
        ...
        **kwargs: dict
        ...
        grid_shape = np.asarray(np.floor((self.grid_size[:, 1] - self.grid_size[:, 0]) / self.grid_step), dtype=np.int64)
    ...
    def pos_to_index(self, position: Vector, relative: bool = False) -> Tuple[int, int]:
        ...
        if self.align_to_vehicle_axes:
            c, s = (
                np.cos(self.observer_vehicle.heading),
                np.sin(self.observer_vehicle.heading),
            )
