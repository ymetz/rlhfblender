...
class RoomGrid(MiniGridEnv):
    ...
    # movable boxes can only be placed in a way that they are not in the corner in the beginning
    def getPossibleUnblockDoorPosRight(self, room):
        x_l, y_l = (room.top[0] + 1, room.top[1] + 1)
        x_m, y_m = (room.top[0] + room.size[0] - 1, room.top[1] + room.size[1] - 1)
        pos = (x_m, self._rand_int(y_l + 1, y_m - 1))
        return pos
    ...
    def clear_right_column(self, i, j):
        room = self.get_room(i, j)
        self.grid.set(room.top, None)
    ...
    def add_object(self, i, j, kind=None, color=None):
        ...
        if kind == None:
            kind = self._rand_elem(["key", "ball", "box", "switch"])

        if color == None:
            color = self._rand_color()

        # TODO: we probably want to add an Object.make helper function
        assert kind in ["key", "ball", "box", "switch"]
        if kind == "key":
            obj = Key(color)
        elif kind == "ball":
            obj = Ball(color)
    ...
    def add_door(
        ...
        room.locked = locked
        if unspecified:
            door = UnspecifiedDoor2(color)
        else:
            door = Door(color, is_locked=locked, is_open=opened)

        if movable_blocked:
            pos = self.getPossibleUnblockDoorPosRight(room)
            color = self._rand_color()
            block = MovableBlock(color)
            self.grid.set(pos[0] - 1, pos[1], block)
            block.cur_pos.append(pos[0] - 1)
            block.cur_pos.append(pos[1])
        else:
    ...
    def add_distractors(
        self, i=None, j=None, num_distractors=10, all_unique=True, no_switches=False
    ):
        """
        ...
        while len(dists) < num_distractors:
            ...
            if no_switches:
                ...
                type = self._rand_elem(["key", "ball", "box", "switch"])
            obj = (type, color)
