...
class ExperimentManager:
    ...
    def create_envs(self, n_envs: int, eval_env: bool = False, no_log: bool = False) -> VecEnv:
        ...
        if "procgen" in self.env_name.gym_id:
            ...
            # Split env name to get the task name (e.g. metaworld-sweep-into-v3 as sweep-into-v3)
            metaworld_task_name = self.env_name.gym_id.split("-")[1:]
            metaworld_task_name = "-".join(metaworld_task_name)
        ...
        if self.reward_function is not None and not eval_env:
            env = RewardVecEnvWrapper(
                env,
