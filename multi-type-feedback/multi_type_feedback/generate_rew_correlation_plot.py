...
def generate_correlation_data(
    ...
    for type in rew_fn_types:
        if noise_level > 0.0:
            ...
            # dirty hot fix for now:
            if type == "demonstrative":
                if noise_level == 1.5:
                    true_noise_level = 0.8
                elif noise_level == 3.0:
                    true_noise_level = 0.9
                else:
            ...
            rew_functions.append(
                os.path.join(
                    base_dir,
                    f"{algo}_{env}_{reward_seed}_{type}_{reward_seed}_noise_{true_noise_level}.ckpt",
                )
            )
        elif n_feedback != -1:
            rew_functions.append(
                os.path.join(
                    base_dir,
                    f"{algo}_{env}_{reward_seed}_{type}_{reward_seed}_nfeedback_{n_feedback}.ckpt",
                )
            )
        else:
