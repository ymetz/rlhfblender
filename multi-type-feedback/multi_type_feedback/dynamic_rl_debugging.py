...
from multi_type_feedback.utils import RewardVecEnvWrapper, TrainingUtils, get_project_root
...
class DynamicRLHFDebugging(DynamicRLHF):
    ...
    def init_debug_statistics(self):
        ...
        self.debug_stats = {
            ...
            "rl_performance": {"episode_returns": [], "episode_lengths": [], "timestamps": []},
    ...
    def train_iteration(self, sampling_strategy: str = "random"):
        ...
        self.debug_stats["trajectory_rewards"].extend(
            [(len(self.debug_stats["iteration_metrics"]), i, r) for i, r in enumerate(trajectory_rewards)]
        )
        iteration_metrics["mean_trajectory_reward"] = np.mean(trajectory_rewards)

        # Get feedback based on sampling strategy
        if sampling_strategy == "random":
            feedback, feedback_counts = self.sample_feedback_random(trajectories, initial_states)
        else:  # uncertainty
            feedback, feedback_counts = self.sample_feedback_uncertainty(trajectories, initial_states)
        ...
        for i, feedback_item in enumerate(feedback):
            for feedback_type, feedback_data in feedback_item.items():
                ...
                if feedback_type == "evaluative" and isinstance(feedback_data, tuple) and len(feedback_data) > 1:
                    ...
                    self.debug_stats["feedback_values"][feedback_type].append(
                        (len(self.debug_stats["iteration_metrics"]), i, rating)
                    )

                elif (
                    feedback_type in ["comparative", "descriptive_preference"]
                    and isinstance(feedback_data, tuple)
                    and len(feedback_data) > 1
                ):
                    preference = feedback_data[1]
                    self.debug_stats["feedback_values"][feedback_type].append(
                        (len(self.debug_stats["iteration_metrics"]), i, preference)
                    )
        ...
        for feedback_type, loss in reward_metrics.items():
            if feedback_type in self.debug_stats["reward_model_losses"]:
                self.debug_stats["reward_model_losses"][feedback_type].append(
                    (len(self.debug_stats["iteration_metrics"]), loss)
                )
    ...
    def compute_model_uncertainty(self, trajectory, feedback_type):
        ...
        self.debug_stats["uncertainties"][feedback_type].append((len(self.debug_stats["iteration_metrics"]), uncertainty))
