...
class HumanFeedbackInterface:
    """
    Interface for collecting human feedback through the frontend system.
    Replaces the FeedbackOracle for human-in-the-loop training.
    """
    ...
    def __init__(
        ...
        self.base_session_dir = Path(f"sessions/{session_id}")
        self.base_session_dir.mkdir(parents=True, exist_ok=True)

        # Create directory for candidate segments
        self.candidates_dir = self.base_session_dir / "candidates"
        self.candidates_dir.mkdir(exist_ok=True)

        # Create directory for collected feedback
        self.feedback_dir = self.base_session_dir / "feedback"
        self.feedback_dir.mkdir(exist_ok=True)

        # Status file for communication with frontend
        self.status_file = self.base_session_dir / "feedback_status.json"

        # Initialize status
        self._update_status("idle", "Ready for feedback collection")

    def _update_status(self, status: str, message: str = "", **kwargs):
    ...
    def save_candidate_segments(
        self, trajectories: list[list], initial_states: list[np.ndarray], sampling_strategy: str = "random"
    ) -> str:
        """
        Save candidate trajectory segments using the existing checkpoint system.
        Creates a new checkpoint with episode data that the frontend can load normally.

        Args:
            trajectories: List of trajectory segments
            initial_states: Corresponding initial states
            sampling_strategy: Strategy used for sampling

        Returns:
            Checkpoint identifier for this set of candidates
        """
        # Generate checkpoint step identifier
        checkpoint_step = f"feedback_{self.current_phase}_{int(time.time())}"

        # Convert trajectories to the format expected by the data system
        all_observations = []
        all_actions = []
        all_rewards = []
        all_dones = []
        all_infos = []
        all_probs = []  # Placeholder for action probabilities

        for trajectory in trajectories:
            for step in trajectory:
                obs, action, reward, done = step
                all_observations.append(obs)
                all_actions.append(action)
                all_rewards.append(reward)
                all_dones.append(done)
                all_infos.append({})  # Placeholder info
                # Create uniform probability distribution as placeholder
                if isinstance(self.action_space, gym.spaces.Discrete):
                    all_probs.append(np.ones(self.action_space.n) / self.action_space.n)
                else:
                    all_probs.append(np.array([1.0]))  # For continuous action spaces

        # Create episode data in the standard format
        episode_data = {
            ...
            "renders": np.zeros((len(all_observations), 64, 64, 3), dtype=np.uint8),  # Placeholder renders
        }

        # Save using the standard data generation pattern
        from rlhfblender.utils import process_env_name

        env_name = self.env_name or "UnknownEnv"
        save_file_name = os.path.join(
            process_env_name(env_name), f"{process_env_name(env_name)}_{self.experiment_id}_{checkpoint_step}"
        )

        # Create directory and save episode data
        save_path = os.path.join("data", "saved_benchmarks", save_file_name)
        os.makedirs(os.path.dirname(save_path), exist_ok=True)

        # Save as npz file
        np.savez(f"{save_path}.npz", **episode_data)

        # Process the data like in data_generation.py (split episodes, create videos, etc.)
        self._process_candidate_data(save_file_name, checkpoint_step)

        # Update experiment checkpoint list asynchronously
        import asyncio

        asyncio.create_task(self._update_experiment_checkpoints(checkpoint_step))

        # Update status to notify frontend
        self._update_status(
            "candidates_ready",
            f"Checkpoint {checkpoint_step} with {len(trajectories)} episodes ready for feedback",
            checkpoint_step=checkpoint_step,
            num_episodes=len(trajectories),
            available_feedback_types=self.feedback_types,
        )

        return checkpoint_step

    def _process_candidate_data(self, save_file_name: str, checkpoint_step: str):
        """
        Process candidate data using the same pattern as data_generation.py
        Split into episodes, create videos, thumbnails, etc.
        """
        import cv2
        from rlhfblender.utils.data_generation import split_data, encode_video

        # Load the saved data
        data = np.load(f"data/saved_benchmarks/{save_file_name}.npz", allow_pickle=True)
        episode_data = split_data(data)

        # Process each episode (same as in data_generation.py)
        for episode_idx, _ in enumerate(episode_data["dones"]):
            ...
            # Save rewards
            os.makedirs(f"data/rewards/{os.path.splitext(save_file_name)[0]}", exist_ok=True)
            np.save(
                f"data/rewards/{os.path.splitext(save_file_name)[0]}/rewards_{episode_idx}.npy",
                np.cumsum(episode_data["rewards"][episode_idx]),
            )

            # Create thumbnails (placeholder since we don't have real renders)
            dir_name = f"data/thumbnails/{os.path.splitext(save_file_name)[0]}"
            os.makedirs(dir_name, exist_ok=True)
            # Create a simple placeholder thumbnail
            placeholder_image = np.zeros((128, 128, 3), dtype=np.uint8)
            cv2.imwrite(f"{dir_name}/{episode_idx}.jpg", placeholder_image)

        # Clean up the original file
        os.remove(f"data/saved_benchmarks/{save_file_name}.npz")

    async def _update_experiment_checkpoints(self, checkpoint_step: str):
        """Update the experiment's checkpoint list with the new checkpoint."""
        ...
        # Skip if no experiment ID is set
        if self.experiment_id is None:
            print("Warning: No experiment ID set, skipping checkpoint update")
            return
        ...
        try:
            ...
            exp = await db_handler.get_single_entry(database, Experiment, key=self.experiment_id)
            if exp is None:
                return
            ...
            if checkpoint_step not in existing_checkpoints:
                ...
                # Sort checkpoints (feedback checkpoints will be at the end)
                existing_checkpoints.sort(key=lambda x: (x.startswith("feedback_"), x))
            ...
            print(f"Error updating experiment checkpoints: {e}")

    def wait_for_feedback(self, checkpoint_step: str, timeout: float = 300.0) -> dict | None:
        """
        Wait for human feedback to be processed by the FeedbackTranslator.
        This waits for the /submit_session endpoint to process feedback and create datasets.

        Args:
            checkpoint_step: Identifier for the checkpoint with candidates
            timeout: Maximum time to wait in seconds

        Returns:
            Dictionary with dataset information or None if timeout
        """
        # Wait for the submission file created by /submit_session
        # Since we're using checkpoints now, look for any batch submission file
        start_time = time.time()

        while time.time() - start_time < timeout:
            # Check for any submitted feedback file for this session
            session_dir = Path(f"sessions/{self.session_id}")
            if session_dir.exists():
                # Look for any batch submission file
                for file_path in session_dir.glob(f"batch_*_submitted.json"):
                    try:
                        ...
                        self._update_status(
                            "feedback_received",
                            f"Datasets created for checkpoint {checkpoint_step}",
                            checkpoint_step=checkpoint_step,
                            datasets_created=submission_data.get("datasets_created", []),
                        )
        ...
        self._update_status(
            "feedback_timeout",
            f"Timeout waiting for feedback processing for checkpoint {checkpoint_step}",
            checkpoint_step=checkpoint_step,
        )
        return None

    def get_feedback(
        self, trajectories: list[list], initial_states: list[np.ndarray], feedback_type: str, sampling_strategy: str = "random"
    ) -> tuple[list[dict], dict[str, int]]:
        """
        Main method to get human feedback for trajectories using the existing FeedbackTranslator system.
        ...
        Returns:
            Tuple of (formatted_feedback, feedback_counts)
        """
        # Save candidate segments
        checkpoint_step = self.save_candidate_segments(trajectories, initial_states, sampling_strategy)

        # Wait for feedback processing to complete
        submission_data = self.wait_for_feedback(checkpoint_step)

        if submission_data is None:
            # Timeout - return empty feedback
            return [], {}

        # Read the processed datasets created by /submit_session
        formatted_feedback = []
        feedback_counts = defaultdict(int)

        datasets_created = submission_data.get("datasets_created", [])

        # Load the created datasets and convert to the format expected by the buffer system
        import pickle

        for feedback_type in datasets_created:
            dataset_file = self.base_session_dir / f"{feedback_type}_dataset.pkl"
            if dataset_file.exists():
                try:
                    with open(dataset_file, "rb") as f:
                        feedback_data = pickle.load(f)

                    # Convert the processed feedback to the legacy buffer format
                        ...
                        if feedback_type == "evaluative":
                            formatted_feedback.append(
                                {
                                    "evaluative": {
                                        ...
                                        "rating": 0.5,  # Default rating, will be updated from actual data
                            ...
                            if len(trajectories) >= 2:
                                formatted_feedback.append(
                                    {
                                        "comparative": {
                                            "trajectory_pair": (trajectories[0], trajectories[1]),
                                            "preference": 1,  # Default preference, will be updated from actual data
                                            "initial_states": (
                            ...
                            if len(trajectories) >= 2:
                                formatted_feedback.append(
                                    {
                                        "corrective": {
                                            "original_trajectory": trajectories[0],
                                            "corrected_trajectory": trajectories[1],
                                            "initial_states": (
                                                (initial_states[0], initial_states[1])
                                                if len(initial_states) >= 2
                                                else (None, None)
                    ...
                    print(f"Error loading dataset {feedback_type}: {e}")

        return formatted_feedback, dict(feedback_counts)

    def set_phase(self, phase: int):
        """Update the current phase."""
        self.current_phase = phase
        self._update_status("phase_updated", f"Phase updated to {phase}")
...
class DynamicRLHF:
    def __init__(
        self,
        session_id: str,
        oracle: Any | None = None,  # Keep for compatibility but make optional
        ...
        self.session_id = session_id
        self.oracle = oracle  # Keep for backward compatibility

        # Create a temporary environment to get action space info
        ...
        self.human_feedback = HumanFeedbackInterface(
            ...
            experiment_id=None,  # Will be set later when available
            segment_len=50,  # Default segment length
            env_name=env_name,
    ...
    def _init_rl_agent(self) -> PPO | SAC:
        ...
        if self.exp_manager:
            ...
            temp_env = gym.make(self.env_name, **self.env_kwargs)
    ...
    def _initialize_reward_models_with_random_feedback(self):
        ...
        for batch in range(batches_needed):
            ...
            # For initial feedback, use human feedback interface
            # Note: This may need adjustment based on whether you want human feedback for initial training
            if self.oracle:
                # Use oracle for initial feedback if available (backward compatibility)
                feedback, batch_counts = self.sample_feedback_random(trajectories, initial_states)
            else:
                # Use human feedback for initial training
                feedback, batch_counts = self.sample_feedback_random(trajectories, initial_states)
    ...
    def collect_trajectories(
        ...
        for _ in range(n_trajectories):
            ...
            segment_len = getattr(self.oracle, "segment_len", 50) if self.oracle else 50
    ...
    def sample_feedback_uncertainty(
        ...
        for trajectory, initial_state, uncertainties in zip(trajectories, initial_states, trajectory_uncertainties):
            ...
            # Use human feedback interface instead of oracle
            feedback, counts = self.human_feedback.get_feedback([trajectory], [initial_state], feedback_type)

            all_feedback.extend(feedback)
            for ft, count in counts.items():
                feedback_counts[ft] += count
    ...
    def sample_feedback_random(
        ...
        # Group trajectories by feedback type for more efficient collection
        grouped_trajectories = defaultdict(list)
        grouped_initial_states = defaultdict(list)

        for trajectory, initial_state, feedback_type in zip(trajectories, initial_states, selected_types):
            grouped_trajectories[feedback_type].append(trajectory)
            grouped_initial_states[feedback_type].append(initial_state)

        # Collect feedback for each type
        for feedback_type in grouped_trajectories:
            feedback, counts = self.human_feedback.get_feedback(
                grouped_trajectories[feedback_type], grouped_initial_states[feedback_type], feedback_type
            )
...
def main():
    ...
    drlhf = DynamicRLHF(
        session_id=f"{args.algorithm}_{args.environment}_{reward_model_type}_{args.seed}",
        oracle=oracle,
        env_name=args.environment,
        env_kwargs=args.environment_kwargs,
        algorithm=args.algorithm,
        feedback_types=args.feedback_types,
        n_feedback_per_iteration=args.n_feedback_per_iteration,
        feedback_buffer_size=args.feedback_buffer_size,
